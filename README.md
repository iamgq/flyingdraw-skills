...
Give your AI Agent (e.g. Coding Agent) a live drawing canvas — it learns to draw via a skill file you drop into your project, and diagrams appear in your browser in real time.
...
AI coding assistants only output text natively. FlyingDraw gives them a **visual output channel** — a live whiteboard they can draw on in real time.

You say `"wireframe the onboarding flow"`. Your agent reads the skill file, generates valid diagram JSON, pushes it to the API, and the diagram appears in your browser instantly — no page reload. You can then annotate or edit the canvas yourself, and the agent can read back what you drew and continue from there.
...
3. It fetches `skill.md` from this repo → learns the diagram JSON format, design rules, and API workflow in one pass
...
- Diagram JSON element format reference
